...
class M_pengguna extends CI_Model{
	...
	function get_all_pengguna(){
		$hsl=$this->db->query("SELECT tbl_pengguna.*,IF(pengguna_jenkel='L','Laki-Laki','Perempuan') AS jenkel FROM tbl_pengguna where tbl_pengguna.pengguna_level = 1 or tbl_pengguna.pengguna_level=3");
		return $hsl;	
	}
	...
	function get_user_image($kode) {
        $this->db->select('pengguna_photo');
        $this->db->from('tbl_pengguna'); // Ganti dengan nama tabel pengguna yang sesuai
        $this->db->where('pengguna_id', $kode);
        $query = $this->db->get();

        if ($query->num_rows() > 0) {
            $result = $query->row();
            return $result->pengguna_photo;
        }

        return null; // Return null jika tidak ada gambar
    }
