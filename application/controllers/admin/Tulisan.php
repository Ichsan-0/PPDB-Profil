...
class Tulisan extends CI_Controller{
	...
	function get_edit(){
		...
		// Cek apakah kategori 7 atau 8 sudah ada di tulisan
		$kategori_ids = [7, 8];
		$count = $this->m_tulisan->count_kategori_in_tulisan($kategori_ids);
		
		if ($count > 0) {
			// Kategori 7 atau 8 sudah digunakan
			$exclude_ids = $kategori_ids;
		} else {
			// Kategori 7 atau 8 belum digunakan
			$exclude_ids = [];
		}
		
		$this->data['kat'] = $this->m_kategori->get_all_kategori($exclude_ids);
